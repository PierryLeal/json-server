...
const local = locations.find((loc) => loc.id === Number(id));

const imageElement = document.getElementById("product-image");

imageElement.src = local.image[0].image;
imageElement.alt = local.name;

document.getElementById("product-title").textContent = local.name;
document.getElementById("product-description").textContent = local.description;
document.getElementById("localization").textContent = local.location;
document.getElementById("register-date").textContent = local.registerDate;

local.image.map(function (item) {
  document.querySelector("#selling-items").insertAdjacentHTML(
    "beforeend",
    `<div class="listing-album-item-styled"  > 
        <div>
          <img style="max-height:100%; border-radius: 5px;" src=${item.image} alt="">
...
const abrirModalBtn = document.getElementsByClassName(
  "listing-album-item-styled"
);

const meuModal = document.getElementById("modal");
const fecharModalBtn = document.getElementById("close-modal");

Array.from(abrirModalBtn).forEach(function (btn, index) {
...
local.image.map(function (image, index) {
  document
...
local.image.map(function (item, index) {
  document.querySelector("#carrousel-itens").insertAdjacentHTML(
    ...
    `<div class="carousel-item ` +
      ...
      ` h-100" >
        ...
        <img class="d-block" src=${item.image}  alt="Primeiro Slide" style="border-radius: 10px">     
        </div>
          <span style="color: #ffffff; text-align: center; margin-top: 1.5rem">${item.description}</span>
        </div>
        `
  );
});
